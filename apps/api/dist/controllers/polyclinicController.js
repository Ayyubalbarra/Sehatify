...
class PolyclinicController {
    ...
    async getAllPolyclinics(req, res, next) {
        try {
            ...
            const polyclinicsWithStats = await Promise.all(polyclinics.map(async (poly) => ({ ...poly, stats: await this.getPolyclinicStats(poly._id) })));
    ...
    async getPolyclinicById(req, res, next) {
        try {
            const polyclinic = await Polyclinic_1.default.findById(req.params.id).lean();
            if (!polyclinic) {
            ...
            const [stats, doctors] = await Promise.all([
                ...
                User_1.default.find({ role: 'doctor', specialization: polyclinic.name, isActive: true }).select("name").lean()
            ]);
    ...
    async createPolyclinic(req, res, next) {
        try {
            const polyclinicData = { ...req.body, polyclinicId: (0, modelHelpers_1.generatePolyclinicId)(), createdBy: req.user?.userId };
    ...
    async updatePolyclinic(req, res, next) {
        try {
            ...
            const polyclinic = await Polyclinic_1.default.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
            if (!polyclinic) {
                return res.status(404).json({ success: false, message: "Poliklinik tidak ditemukan" });
            }
    ...
    async getPolyclinicStats(polyclinicId) {
        try {
            const [monthlyVisits, activeDoctors] = await Promise.all([
                Visit_1.default.countDocuments({ polyclinicId, visitDate: { $gte: new Date(new Date().setDate(1)) } }),
                User_1.default.countDocuments({ role: 'doctor', polyclinicId, isActive: true }) // Ini mungkin perlu disesuaikan jika dokter tidak langsung terhubung ke poliklinik
            ]);
